    ...
    def getOrderInfo(self):
        ...
        try:
            ...
            if self.crawlWrongDate:
                try:
                    ...
                    while date_list_len > 0:
                        startDate = date_list[date_list_len-1].to_pydatetime().date().strftime('%m/%d/%Y')  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
                        endDate = date_list[date_list_len-1].to_pydatetime().date().strftime('%m/%d/%Y')  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
            ...
            if self.data_crawl_type == 'CL':  # 存量数据抓取
                ...
                try:
                    today_str = datetime.date.today().strftime('%Y-%m-%d')
                    sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_us a where zone = '%s' and substr(create_date,1,10) = '%s'" % (self.zone, today_str)
                    order_date_max = pd.read_sql(sqlcmd, self.dbconn)
                    if (len(order_date_max) > 0 and order_date_max is not None and order_date_max['order_date'][
                        0] is not None):
                        begin = order_date_max['order_date'][0].to_pydatetime().date() - datetime.timedelta(days=1)
            ...
            if self.data_crawl_type == 'CL':
                ...
                for date_str in self.need_to_crawl_date_list:
                    startDate = datetime.datetime.strptime(date_str, '%Y-%m-%d').strftime('%m/%d/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                    endDate = startDate
    ...
    def parsePageInfo(self, driver, sub_zone, startDate, order_date_str):
        ...
        try:
            if sub_zone != self.now_sub_zone:
                ...
                total_cnt_usl_query = "delete from amazon_report_total_orders_source_us where zone = '%s' and sub_zone = '%s' and order_date = '%s'" % (self.zone, sub_zone, order_date_str)
