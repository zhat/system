...
logging.basicConfig(level=logging.ERROR,
                ...
                filename='AmazonManagerOrdersCrawlCA.log',
                filemode='w')

_LOGGING = logging.getLogger('AmazonManagerOrdersCrawlCA.py')
...
class AmazonOrderManagerCrawl():
    ...
    def need_to_crawl(self):
        try:
            ...
            sqlcmd = "select min(substr(order_date,1,10)) as order_date from amazon_report_manager_orders_ca a where zone = '%s' and substr(create_date,1,10) = '%s'" % (
            self.zone, today_str)
            ...
            crawl_day_end = (datetime.date.today() - datetime.timedelta(days=self.crawl_days)).strftime('%Y-%m-%d')

            if order_date_list_len > 0 and today_str == crawl_day_end:
                return False
            else:
                return True
        ...
        try:
            ...
            if self.crawlWrongDate:
                try:
                    ...
                    insert_exists_date = "insert into amazon_report_total_orders_source_ca(zone, order_date, total_cnt) " \
                                         "select '" + self.zone + "', order_date, total_cnt " \
                                                                  "  from amazon_report_order_manager_recrawl_ca a " \
                                                                  " where not exists (select 1 from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_ca group by 1,2 order by 1,2) b" \
                                                                  "  where a.zone = b.zone" \
                                                                  "    and a.order_date = b.order_date)"
                    self.cur.execute(insert_exists_date)
                    self.dbconn.commit()

                    clear_wrong_data = "delete from amazon_report_order_manager_recrawl_ca"
                    self.cur.execute(clear_wrong_data)
                    self.dbconn.commit()

                    create_recrawl_list = "insert into amazon_report_order_manager_recrawl_ca(order_date, zone, cnt, total_cnt) " \
                                          "select a.order_date , a.zone , case when b.cnt is not null then a.total_cnt - b.cnt else a.total_cnt end as cnt, a.total_cnt" \
                                          "  from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_ca group by 1,2 order by 1,2) a" \
                                          "  left join (select order_date,zone,count(distinct order_id) as cnt from amazon_report_manager_orders_ca group by 1,2) b" \
                    ...
                    delete_exists_data_1 = "delete from amazon_report_manager_orders_ca " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_ca)" \
                                           "   and zone = '"+self.zone+"'"
                    self.cur.execute(delete_exists_data_1)
                    self.dbconn.commit()

                    delete_exists_data_2 = "delete from amazon_report_total_orders_source_ca " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_ca)" \
                                           "   and zone = '"+self.zone+"'"
                    self.cur.execute(delete_exists_data_2)
                    self.dbconn.commit()

                    select_recrawl_list_query = "select distinct order_date from amazon_report_order_manager_recrawl_ca"
                    ...
                    while date_list_len > 0:
                        if self.zone in ['US', 'CA']:
                            ...
                            startDate = date_list[date_list_len - 1].to_pydatetime().date().strftime(
                                '%d/%m/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                            endDate = date_list[date_list_len - 1].to_pydatetime().date().strftime(
                                '%d/%m/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
            ...
            if self.data_crawl_type == 'CL':  # 存量数据抓取
                try:
                    sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_ca a where zone = '" + self.zone + "'"
                    order_date_max = pd.read_sql(sqlcmd, self.dbconn)
                    if (len(order_date_max) > 0 and order_date_max is not None and order_date_max['order_date'][
                        0] is not None):
                        begin = order_date_max['order_date'][0].to_pydatetime().date() - datetime.timedelta(
                            days=1)
                    else:
                ...
                try:
                    today_str = datetime.date.today().strftime('%Y-%m-%d')
                    sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_ca a where zone = '%s' and substr(create_date,1,10) = '%s'" % (
                        self.zone, today_str)
                    order_date_max = pd.read_sql(sqlcmd, self.dbconn)
                    if (len(order_date_max) > 0 and order_date_max is not None and order_date_max['order_date'][
                        0] is not None):
            ...
            while d >= end and self.tryCnt>0:
                ...
                if self.zone in ['US', 'CA']:
                    startDate = d.strftime('%m/%d/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                    endDate = begin.strftime('%m/%d/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                else:
                    startDate = d.strftime('%d/%m/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                    endDate = begin.strftime('%d/%m/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
    ...
    def parsePageInfo(self, driver, sub_zone, startDate, order_date_str):
        ...
        try:
            if sub_zone != self.now_sub_zone:
                ...
                total_cnt_query = "insert into amazon_report_total_orders_source_ca(zone, sub_zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s, %s)"
                ...
                now_order_cnt_str = tree.xpath(
                    '//*[@id="myo-table"]/table/tbody/tr[1]/td/table/tbody/tr/td[1]/strong[2]/text()')
                now_order_cnt = int(now_order_cnt_str[0])
                total_cnt_query = "insert into amazon_report_total_orders_source_ca(zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s)"
        ...
        for order in orders:
            ...
            sqli = "insert into amazon_report_manager_orders_ca(order_id, cust_id, zone, order_date, create_date) values(%s, %s, %s, %s, %s)"
...
amzCrawl = AmazonOrderManagerCrawl("CA", 16, "ZL", 48)  # 每次启动跑16天的数据，截至到当天往前推62天
amzCrawl.crawlWrongDate = False

if amzCrawl.data_crawl_type == 'ZL':
    continue_flag = amzCrawl.need_to_crawl()
    if not continue_flag:
        sys.exit(0)

amzCrawl.getOrderInfo()
executor = AutoUpdateData()
executor._update_data('ca')
executor._exit()
