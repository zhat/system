...
logging.basicConfig(level=logging.ERROR,
                ...
                filename='AmazonManagerOrdersCrawlFR.log',
                filemode='w')

_LOGGING = logging.getLogger('AmazonManagerOrdersCrawlFR.py')
        ...
        try:
            ...
            if self.crawlWrongDate:
                try:
                    ...
                    insert_exists_date = "insert into amazon_report_total_orders_source_fr(zone, order_date, total_cnt) " \
                                         "select '" + self.zone + "', order_date, total_cnt " \
                                                                  "  from amazon_report_order_manager_recrawl_fr a " \
                                                                  " where not exists (select 1 from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_fr group by 1,2 order by 1,2) b" \
                                                                  "  where a.zone = b.zone" \
                                                                  "    and a.order_date = b.order_date)"
                    self.cur.execute(insert_exists_date)
                    self.dbconn.commit()

                    clear_wrong_data = "delete from amazon_report_order_manager_recrawl_fr"
                    self.cur.execute(clear_wrong_data)
                    self.dbconn.commit()

                    create_recrawl_list = "insert into amazon_report_order_manager_recrawl_fr(order_date, zone, cnt, total_cnt) " \
                                          "select a.order_date , a.zone , case when b.cnt is not null then a.total_cnt - b.cnt else a.total_cnt end as cnt, a.total_cnt" \
                                          "  from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_fr group by 1,2 order by 1,2) a" \
                                          "  left join (select order_date,zone,count(distinct order_id) as cnt from amazon_report_manager_orders_fr group by 1,2) b" \
                    ...
                    delete_exists_data_1 = "delete from amazon_report_manager_orders_fr " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_fr)" \
                                           "   and zone = '"+self.zone+"'"
                    self.cur.execute(delete_exists_data_1)
                    self.dbconn.commit()

                    delete_exists_data_2 = "delete from amazon_report_total_orders_source_fr " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_fr)" \
                                           "   and zone = '"+self.zone+"'"
                    self.cur.execute(delete_exists_data_2)
                    self.dbconn.commit()

                    select_recrawl_list_query = "select distinct order_date from amazon_report_order_manager_recrawl_fr"
            ...
            try:
                # sqlcmd = "select max(order_date) as order_date from amazon_report_manager_orders_fr a where zone = '" + self.zone + "'"
                sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_fr a where zone = '" + self.zone + "'"
    ...
    def parsePageInfo(self, driver, sub_zone, startDate, order_date_str):
        ...
        try:
            if sub_zone != self.now_sub_zone:
                ...
                total_cnt_query = "insert into amazon_report_total_orders_source_fr(zone, sub_zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s, %s)"
                ...
                now_order_cnt_str = tree.xpath(
                    '//*[@id="myo-table"]/table/tbody/tr[1]/td/table/tbody/tr/td[1]/strong[2]/text()')
                now_order_cnt = int(now_order_cnt_str[0])
                total_cnt_query = "insert into amazon_report_total_orders_source_fr(zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s)"
        ...
        for order in orders:
            ...
            sqli = "insert into amazon_report_manager_orders_fr(order_id, cust_id, zone, order_date, create_date) values(%s, %s, %s, %s, %s)"
...
amzCrawl = AmazonOrderManagerCrawl("FR", 15)
