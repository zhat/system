...
def _search_by_name(zone,name,asin):
    order_of_name = []
    order_list = AmazonOrderRemote.objects.using('remote').filter(platform=zone). \
        filter(customer_name__iexact=name).all().order_by("-purchase_at")
    for order in order_list:
        ...
        if asin and asin not in order_item_asin_list:
            continue
        order_dict = {
            'zone': order.platform,
...
def search_by_name(request):
    ...
    order_of_name = []
    if name and zone:
        order_of_name = _search_by_name(zone=zone,name=name,asin=asin)
