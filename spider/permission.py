...
def perm_check(request, *args, **kwargs):
    ...
    if url_name:
        ...
        # 操作数据库
        print(url_name,url_method,url_args_list)
        get_perm = Permission.objects.filter(Q(url=url_name) and Q(per_method=1)
                        and Q(argument_list='qw'))
        #print(get_perm)
        if get_perm:
            for i in get_perm:
                perm_name = i.name
                perm_str = 'spider.%s' % perm_name
                print(perm_name)
                print(perm_str)
