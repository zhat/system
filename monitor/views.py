...
def get_data(date_list,shop_name_list,zone,field):
    tuples = [(shop_name, date) for shop_name in shop_name_list for date in date_list]
    ...
    for feedback in feedback_list:
        ...
        if feedback[field]:
            ...
            data_frame.loc[(feedback['shop_name'], feedback['date'].strftime("%Y-%m-%d")), field] = 0
    for shop_name in shop_name_list:
        field_data_list.append({'shop_name': shop_name, field: list(map(int, data_frame.loc[shop_name][field].values))})
...
def feedback(request):
    ...
    # 当天的各站点数据表格显示
    feedback_table_data = []
    feedback_count_list = FeedbackInfo.objects.filter(date=today_str).filter(zone=zone)
    ...
    week_date_list = [(date-timedelta(days=1)).strftime("%Y-%m-%d") for date in days]        #日期列表
    ...
    months= [(date-timedelta(days=1)).strftime("%Y-%m") for date in dates]  #每月1号的数据是上个月的统计
    months.reverse()
