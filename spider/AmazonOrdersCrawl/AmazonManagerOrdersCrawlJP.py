...
logging.basicConfig(level=logging.ERROR,
                    ...
                    filename='AmazonManagerOrdersCrawlUS.log',
                    filemode='w')

_LOGGING = logging.getLogger('AmazonManagerOrdersCrawlUS.py')
...
class AmazonOrderManagerCrawl():
    ...
    def need_to_crawl(self):
        try:
            ...
            sqlcmd = "select distinct substr(order_date,1,10) as order_date from amazon_report_manager_orders_us a where zone = '%s' and substr(create_date,1,10) = '%s'" % (
                self.zone, today_str)
    ...
    def getOrderInfo(self):
        ...
        driver = webdriver.Chrome(
            executable_path='D:\\andy_yang\\projects\\AmazonPaymentGenerator\\drive\\chromedriver.exe',
        ...
        try:
            ...
            if self.crawlWrongDate:
                try:
                    ...
                    insert_exists_date = "insert into amazon_report_total_orders_source_us(zone, order_date, total_cnt) " \
                                         "select '" + self.zone + "', order_date, total_cnt " \
                                                                  "  from amazon_report_order_manager_recrawl_us a " \
                                                                  " where not exists (select 1 from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_us group by 1,2 order by 1,2) b" \
                                                                  "  where a.zone = b.zone" \
                                                                  "    and a.order_date = b.order_date)"
                    self.cur.execute(insert_exists_date)
                    self.dbconn.commit()

                    clear_wrong_data = "delete from amazon_report_order_manager_recrawl_us"
                    self.cur.execute(clear_wrong_data)
                    self.dbconn.commit()

                    create_recrawl_list = "insert into amazon_report_order_manager_recrawl_us(order_date, zone, cnt, total_cnt) " \
                                          "select a.order_date , a.zone , case when b.cnt is not null then a.total_cnt - b.cnt else a.total_cnt end as cnt, a.total_cnt" \
                                          "  from (select zone,order_date,sum(total_cnt) as total_cnt from amazon_report_total_orders_source_us group by 1,2 order by 1,2) a" \
                                          "  left join (select order_date,zone,count(distinct order_id) as cnt from amazon_report_manager_orders_us group by 1,2) b" \
                    ...
                    delete_exists_data_1 = "delete from amazon_report_manager_orders_us " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_us)" \
                                           "   and zone = '" + self.zone + "'"
                    self.cur.execute(delete_exists_data_1)
                    self.dbconn.commit()

                    delete_exists_data_2 = "delete from amazon_report_total_orders_source_us " \
                                           " where order_date in (select distinct order_date from amazon_report_order_manager_recrawl_us)" \
                                           "   and zone = '" + self.zone + "'"
                    self.cur.execute(delete_exists_data_2)
                    self.dbconn.commit()

                    select_recrawl_list_query = "select distinct order_date from amazon_report_order_manager_recrawl_us"
                    ...
                    while date_list_len > 0:
                        startDate = date_list[date_list_len - 1].to_pydatetime().date().strftime(
                            '%m/%d/%Y')  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
                        endDate = date_list[date_list_len - 1].to_pydatetime().date().strftime(
                            '%m/%d/%Y')  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
                        order_date_str = date_list[date_list_len - 1].to_pydatetime().date().strftime(
                            '%Y-%m-%d %H:%M:%S')
            ...
            if self.data_crawl_type == 'CL':  # 存量数据抓取
                try:
                    sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_us a where zone = '" + self.zone + "'"
                    order_date_max = pd.read_sql(sqlcmd, self.dbconn)
                ...
                try:
                    today_str = datetime.date.today().strftime('%Y-%m-%d')
                    sqlcmd = "select min(order_date) as order_date from amazon_report_manager_orders_us a where zone = '%s' and substr(create_date,1,10) = '%s'" % (
                    self.zone, today_str)
                    order_date_max = pd.read_sql(sqlcmd, self.dbconn)
                    if (len(order_date_max) > 0 and order_date_max is not None and order_date_max['order_date'][
                        0] is not None):
            ...
            if self.data_crawl_type == 'CL':
                ...
                while d >= end and self.tryCnt > 0:
                    ...
                    startDate = d.strftime('%m/%d/%Y')  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
                    endDate = begin.strftime("%m/%d/%Y")  # 美国站、加拿大、日本是'%m/%d/%Y'，德国站点是%d%m%Y
                ...
                for date_str in self.need_to_crawl_date_list:
                    startDate = datetime.datetime.strptime(date_str, '%Y-%m-%d').strftime(
                        '%m/%d/%Y')  # 美国站和其他站点的日期格式不同，其他站点是%d%m%Y
                    endDate = startDate
    ...
    def crawlByExactDateAndZone(self, driver, startDate, endDate, sub_zone_url, sub_zone, order_date_str):
        ...
        try:
            ...
            if 'Too Many Orders Found' in search_result_str and (
                ...
                for salesChannel in salesChannels:
                    if salesChannel != 'sitesall':
                        ...
                        if 'No Order Found' in search_result_str and (
                                    style is None or len(style) == 0 or 'display' not in style[0]):
                            continue

                        sub_zone = amzid_marketplace_dict[salesChannel]
                        self.crawlBySalesChannel(driver, startDate, endDate, sub_zone_url, sub_zone, order_date_str,
                                                 salesChannel)
            elif 'No Order Found' in search_result_str and (
                        style is None or len(style) == 0 or 'display' not in style[0]):
                return
            else:
                pass
    ...
    def parsePageInfo(self, driver, sub_zone, startDate, order_date_str):
        ...
        try:
            if sub_zone != self.now_sub_zone:
                ...
                total_cnt_usl_query = "delete from amazon_report_total_orders_source_us where zone = '%s' and sub_zone = '%s' and order_date = '%s'" % (
                self.zone, sub_zone, order_date_str)
                self.cur.execute(total_cnt_usl_query)

                total_cnt_query = "insert into amazon_report_total_orders_source_us(zone, sub_zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s, %s)"
                ...
                total_cnt_query = "insert into amazon_report_total_orders_source_us(zone, order_date, total_cnt, create_date) values(%s, %s, %s, %s)"
        ...
        for order in orders:
            ...
            # order_id_exists_sql = "select order_id from amazon_report_manager_orders_us where order_id = '%s'" % order_id
            ...
            #     pass
            # else:
            #     sqli = "insert into amazon_report_manager_orders_us(order_id, cust_id, zone, order_date, create_date) values(%s, %s, %s, %s, %s)"
    ...
    def systemErrorHandle(self, driver, waitSeconds):
        try:
            content = driver.page_source
            tree = etree.HTML(content)
            _myoLO_pleaseWaitExtendedMessage = tree.xpath(
                '//*[@id="_myoV2_AjaxGenericSysErrorMessagePlaceholder"]//text()')
...
amzCrawl = AmazonOrderManagerCrawl("US", 16, "ZL", 48)  # 每次启动跑16天的数据，截至到当天往前推62天
